...
public partial class admin_activity_verification : System.Web.UI.Page
{    
    ...
    public static Int64 current_activity_id = 0;
    public byte current_media_id = 0;

    public Int32 start_row;
    public Int32 end_row;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            SessionState._PaginationSession = new PaginationSession();
            SessionState._PaginationSession.start_row = 1;
            SessionState._PaginationSession.end_row = 1;
            FirstPos();
        }
        else
        {
    ...
    private void GetActivity()
    {
        ...
        cmd.Parameters.AddWithValue("@brand_id", b_id);

        if (SessionState._Campaign != null)
        {
            if (SessionState._Campaign.campaign_id != 0) cmd.Parameters.AddWithValue("@campaign_id", SessionState._Campaign.campaign_id);

            if (SessionState.EditId != 0) cmd.Parameters.AddWithValue("@activity_id", SessionState.EditId);
        }
        
        cmd.Parameters.AddWithValue("@start_row", SessionState._PaginationSession.start_row);
        cmd.Parameters.AddWithValue("@end_row", SessionState._PaginationSession.end_row);
        ...
        {
            div_MainContent.Visible = false;
            div_NoContent.Visible = true;
        }
    }
    private void NullAllRepeaters()
    {
        RepTab_PreviousActivities.DataSource = null;
        RepTab_PreviousActivities.DataBind();
        RepTab_FBCurrentActivity.DataSource = null;
        RepTab_FBCurrentActivity.DataBind();
        RepTab_GCurrentActivity.DataSource = null;
        RepTab_GCurrentActivity.DataBind();
        RepTab.DataSource = null;
        RepTab.DataBind();
        RepTab_CampaignDetails.DataSource = null;
    ...
    protected void RepTab_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        {   
            ...
            foreach (string str in medias.Split('~'))
            {
                ...
                media_str += "<small class=\"label label-primary\">" + str + "</small>&nbsp;";
            }
    ...
    protected void btn_NextActivity_Click(object sender, EventArgs e)
    {
        LinkButton btn = (LinkButton)(sender);
        SessionState._PaginationSession.start_row = SessionState._PaginationSession.start_row + 1;
        SessionState._PaginationSession.end_row = SessionState._PaginationSession.start_row;
        GetActivity();
    }
    protected void btn_PrevActivity_Click(object sender, EventArgs e)
    {
        LinkButton btn = (LinkButton)(sender);
        SessionState._PaginationSession.start_row = SessionState._PaginationSession.start_row - 1;
        SessionState._PaginationSession.end_row = SessionState._PaginationSession.start_row;
    ...
    protected void lnkbtnApprove_Click(object sender, EventArgs e)
    {
        byte status = 2;
        LinkButton btn = (LinkButton)(sender);
        //string test = Convert.ToString(btn.CommandArgument);
        //Int64 t = current_activity_id;
        SqlCommand cmd = new SqlCommand("sp_brandyy_UpdateActivityRewardStatus");
        cmd.Parameters.AddWithValue("@activity_id", current_activity_id);
