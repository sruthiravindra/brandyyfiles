...
{    
    ...
    {
        CreateInstance();
        if (SessionState._BrandyyAdmin == null)
        {
    ...
    protected void lnkbtnApprove_Click(object sender, EventArgs e)
    {
        byte status = 2;
        LinkButton btn = (LinkButton)(sender);
        SqlCommand cmd = new SqlCommand("sp_IchooseIT_UpdateActivityRewardStatus");
        cmd.Parameters.AddWithValue("@activity_id", Convert.ToInt64(btn.CommandArgument));
