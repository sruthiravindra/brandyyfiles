...
{    
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect(SessionState.WebsiteURLAdmin + "brandcampaigns.aspx");
        }
    ...
    private string LoadImageGallery()
    {
        ...
        string imgs = "";
        //string path = Server.MapPath("~/brands/campaigns/16/" + Convert.ToString(SessionState._BrandyyAdmin.brand_id) + "/" + Convert.ToString(SessionState._Campaign.campaign_id));
        //if (Directory.Exists(path))
        //{
        //    string[] filePaths = Directory.GetFiles(Server.MapPath("~/brands/campaigns/16/" + Convert.ToString(SessionState._BrandyyAdmin.brand_id) + "/" + Convert.ToString(SessionState._Campaign.campaign_id)));            
        //    foreach (string filePath in filePaths)
        //    {
        //        string fileName = Path.GetFileName(filePath);
        //        imgs += "<li><img class='thumbnail' src='" + "campaigns/16/" + Convert.ToString(SessionState._BrandyyAdmin.brand_id) + "/" + Convert.ToString(SessionState._Campaign.campaign_id) + "/" + fileName + "' /></li>";            
        //    }
        //}
    ...
    private void PickALuckyDrawUser()
    {
        SqlCommand cmd = new SqlCommand("sp_IChooseIT_UpdateActivityWinner");
        cmd.Parameters.AddWithValue("@campaign_id", Convert.ToInt64(SessionState._Campaign.campaign_id));
        cmd.Parameters.AddWithValue("@returnregid", SqlDbType.BigInt).Direction = ParameterDirection.Output;
        ConnObj.GetDataSet(cmd);

        if ((ConnObj.IsSuccess) && (ConnObj.DataSet.Tables.Count > 0) && (ConnObj.DataSet.Tables[0].Rows.Count > 0))
        {
            string id = cmd.Parameters["@returnregid"].Value.ToString();

            if (id == "")
            {
                // display couldnt locate a winner
                divLuckyDrawWinner.Visible = true;
                divLuckyDrawWinner.InnerHtml = "<span class='label label-danger'>Could not locate a winner</span>";
            }

            Int64 reg_uid = Convert.ToInt64(id);

            cmd = new SqlCommand("sp_Select_IChooseIT_Registrations");
            cmd.Parameters.AddWithValue("@reg_uid", Convert.ToInt64(reg_uid));
            ConnObj.GetDataSet(cmd);

            if ((ConnObj.IsSuccess) && (ConnObj.DataSet.Tables.Count > 0) && (ConnObj.DataSet.Tables[0].Rows.Count > 0))
            {
                string enddate = "";
                if (SessionState._Campaign.campaign_status == 1)
                {
                    if (DateTime.Compare(SessionState._Campaign.campaign_end, DateTime.Now) >= 0)
                    {
                        enddate = "Your campaign is still running you SHOULD ensure that a draw is performed after the campaign ends so that all participants can enter the draw";
                    }
                }
                ...
                lnk.ID = "lnkWinner";
                lnk.Text = "Click Here to View " + dr["reg_fname"] + "'s Activities";
                lnk.Click += new EventHandler(lnk_UserActivity_Click);
                lnk.Command += new CommandEventHandler(lnk_UserActivity_Click);
                lnk.CommandArgument = Convert.ToString(reg_uid);
                divLuckyDrawWinner.Controls.Add(lnk);
                ...
                createDiv.InnerHtml = "<div class=\"col-lg-6\"><h4 class=\"box-heading\">WINNER</h4>"
                    + "<div class=\"media\"><a href=\"#\" class=\"pull-left\"><img data-src=\"" + dr["profile_image_url"] + "\" alt=\"avatar\""
                + "class=\"media-object\" src=\"" + dr["profile_image_url"] + "\" style=\"width: 64px; height: 64px;\"></a>"
                + "<div class=\"media-body\"><h4 class=\"media-heading\">" + dr["reg_name"] + "</h4>" + dr["reg_email"] + "<br>Is the Winner of the lucky draw conducted on " + DateTime.Now + "</div>"
                + "</div>"
                + "<div></br></br><span class='label label-red'>" + enddate + "</span></div></div>";
                divLuckyDrawWinner.Controls.Add(createDiv);
            }
            else
            {
    ...
    protected void lnk_UserActivity_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect(SessionState.WebsiteURLAdmin + "activity-listing.aspx");
    }
    ...
    protected void lnkPickLuckydraw_Click(object sender, EventArgs e)
    {
        PickALuckyDrawUser();
    }
